...
def parse(sdateurl,name, header1):

	print("The Type ofsdateurl {}".format(sdateurl))
...
def main():
	...
	for num,i in enumerate(state_list):
		if(i.rsplit('/')[-1] == 'Ellicott+City'):
			start = num
		if(i.rsplit('/')[-1] == 'Montgomery+Village'):
			end = num

	state_list = state_list[start:end+1]
	...
	for state in state_list:
		...
		pool.starmap(parse,zip((i for i in dateurl), repeat(name),repeat(header1)))
		pool.close()
		pool.join()
